...
export class PrismaServiceManagementRepository {
  ...
  async createService(shopId: string, input: {
    name: string;
    description?: string;
    durationMin: number;
    price: number;
    ...
    return prisma.shopService.create({
      data: {
        ...
        category: input.category as Prisma.ServiceCategory,
  ...
  async updateService(serviceId: string, input: {
    name?: string;
    description?: string;
    durationMin?: number;
    price?: number;
  ...
  async replaceComboItems(comboServiceId: string, items: Array<{ serviceId: string; quantity: number }>) {
