...
import type { OrphanedChair, RecoveryConfig, RecoveryStats } from "./recovery-types.js";
...
export class ChairRecoveryWorker {
  ...
  constructor(
    ...
    intervalMs: number = 10 * 60 * 1000 // Default: 10 minutes
  ) {
   ...
   */
  async detectOrphanedChairs(): Promise<OrphanedChair[]> {
    const orphanedChairs: OrphanedChair[] = [];
   ...
   */
  async detectOrphanedChairsForShop(shopId: string): Promise<OrphanedChair[]> {
    const orphanedChairs: OrphanedChair[] = [];
    const now = new Date();
    ...
    for (const chair of occupiedChairs) {
      ...
      let reason: "NO_ACTIVE_BOOKING" | "BOOKING_COMPLETED" | "BOOKING_CANCELLED" | "TIMEOUT" = "TIMEOUT";
   ...
   */
  async recoverOrphanedChairs(orphanedChairs: OrphanedChair[]): Promise<number> {
    ...
    for (const orphanedChair of orphanedChairs) {
      await this.transactionManager.executeRecovery(
        () => this.transactionManager.recoverOrphanedChair(
          orphanedChair.chairId,
          orphanedChair.shopId,
          orphanedChair.reason
        ),
        "CHAIR_RECOVERED",
        orphanedChair.shopId,
  ...
  async getStats(): Promise<{
    totalOccupiedChairs: number;
    orphanedChairs: number;
    ...
    const orphanedChairs = occupiedChairs.filter(
      c => c.activeServiceStart && c.activeServiceStart < timeoutThreshold
    );
