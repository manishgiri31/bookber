import type { Prisma, QueueLane, ServiceCategory } from "@prisma/client";
...
import type {
  BookingWaitSnapshot,
  CategoryAverages,
  LaneRecalculationResult,
  WaitEstimatesResponse,
  WaitRecalcTrigger,
  WaitShopConfig
...
import {
  calculateLaneWaitEstimates,
  type WaitCalculationInput
} from "./wait-time.calculator.js";

const DEFAULT_CONFIG: WaitShopConfig = {
...
type HydratedShop = {
  ...
  reservedChairCount: number;
  walkInChairCount: number;
};

export class WaitTimeEngine {
  private readonly redis: WaitTimeRedisStore;
  readonly persistence: WaitTimePersistence;
   ...
   */
  async getEstimates(shopId: string): Promise<WaitEstimatesResponse> {
    if (!this.hasRedis()) {
  ...
  async recalculateLane(
    shopId: string,
    lane: QueueLane,
    _trigger: WaitRecalcTrigger
  ): Promise<LaneRecalculationResult> {
    if (!this.hasRedis()) {
    ...
    const activeChairs =
      lane === "BOOKBER" ? config.activeReservedChairs : config.activeWalkInChairs;
  ...
  async recalculateShop(shopId: string, trigger: WaitRecalcTrigger): Promise<WaitEstimatesResponse> {
  ...
  async onEnqueue(snapshot: BookingWaitSnapshot): Promise<LaneRecalculationResult> {
    if (this.hasRedis()) {
  ...
  async onDequeue(shopId: string, lane: QueueLane, bookingId: string): Promise<LaneRecalculationResult> {
    if (this.hasRedis()) {
  ...
  async recordBarberDelay(shopId: string, barberId: string, delayMinutes: number): Promise<void> {
  ...
  private async computeAndApply(
    ...
    const barberHistorical = new Map<string, CategoryAverages>();
    const barberRecent = new Map<string, CategoryAverages>();
    const barberState = new Map<string, import("../domain/wait-time.types.js").BarberRuntimeState>();
    const barberSpeedFactors = new Map<string, number>();

    const barberIds = [...new Set(entries.map((e) => e.barberId).filter(Boolean))] as string[];
    await Promise.all(
      barberIds.map(async (barberId) => {
        ...
        barberState.set(
          barberId,
          state ?? { delayMinutes: 0, overrunMinutes: 0, updatedAtMs: Date.now() }
        );
        barberSpeedFactors.set(barberId, barber?.serviceSpeedFactor ?? 1);
      })
    );

    const input: WaitCalculationInput = {
      now: new Date(),
      config: shop.config,
      entries,
      shopHistorical,
  ...
  private async buildSnapshotsForRecalc(
    shopId: string,
    lane: QueueLane,
    bookingIds: string[]
  ): Promise<BookingWaitSnapshot[]> {
    const cached = await this.redis.getBookingSnapshots(bookingIds);
    if (cached.length === bookingIds.length) return cached;
   ...
   */
  private async readLaneEstimatesFromRedis(
    shopId: string,
    lane: QueueLane
  ): Promise<LaneRecalculationResult["estimates"]> {
  ...
  private async ensureShopHydrated(shopId: string): Promise<HydratedShop> {
    ...
    const config: WaitShopConfig = {
      activeReservedChairs: Math.max(
        1,
        shop.bookBerReservedChairCount,
        reservedChairs.filter((c) => c.status === "AVAILABLE" || c.status === "OCCUPIED").length
      ),
      activeWalkInChairs: Math.max(
        1,
        walkInChairs.filter((c) => c.status === "AVAILABLE" || c.status === "OCCUPIED").length
      ),
  ...
  private async recalculateLaneFromDatabase(
    shopId: string,
    lane: QueueLane
  ): Promise<LaneRecalculationResult> {
    const shop = await this.ensureShopHydrated(shopId);
    const activeChairs =
      lane === "BOOKBER" ? shop.config.activeReservedChairs : shop.config.activeWalkInChairs;
    ...
    const estimates = calculateLaneWaitEstimates({
      ...
      config: shop.config,
      entries,
      shopHistorical: buildInitialShopAverages(
        rows.map((r) => r.booking.service)
      ),
      shopRecent: {},
      barberHistorical: new Map(),
      barberRecent: new Map(),
      barberState: new Map(),
      barberSpeedFactors: new Map(
        rows
  ...
  private async getEstimatesFromDatabase(shopId: string): Promise<WaitEstimatesResponse> {
    const [bookBer, walkIn] = await Promise.all([
...
function buildInitialShopAverages(
  services: Array<{ category: ServiceCategory; durationMinutes: number }>
...
function bookingRowToSnapshot(row: {
  ...
  booking: {
    id: string;
    serviceId: string;
    service: { durationMinutes: number; category: ServiceCategory };
  };
}): BookingWaitSnapshot {
  const remaining =
    row.estimatedServiceStart &&
    (row.queueStatus === "IN_SERVICE" || row.queueStatus === "CALLED")
      ? Math.max(
          0,
          row.booking.service.durationMinutes -
  ...
  return {
    ...
    estimatedServiceStartIso:
      row.estimatedServiceStart?.toISOString() ?? new Date().toISOString(),
