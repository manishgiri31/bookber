...
export class QueueNormalizationJob {
   ...
   */
  async run(): Promise<{ normalizedShops: number; normalizedLanes: number }> {
    ...
    for (const shop of shops) {
      ...
      for (const lane of lanes) {
        const needsRebalancing = await this.positionAllocator.needsRebalancing(
          prisma,
          shop.id,
          lane
        );
        const needsNormalization = await this.positionAllocator.needsNormalization(
          prisma,
          shop.id,
          lane
        );
   ...
   */
  async normalizeShop(shopId: string): Promise<{ normalizedLanes: number }> {
    const lanes: QueueLane[] = ["BOOKBER", "WALKIN"];
    let normalizedLanes = 0;
  ...
  async getHealthStats(): Promise<{
    totalShops: number;
    shopsNeedingNormalization: number;
    lanesNeedingNormalization: number;
    maxPosition: number;
    minGap: number;
  }> {
    const shops = await prisma.shop.findMany({
   ...
   */
  async forceNormalizeShop(shopId: string): Promise<{ normalizedLanes: number }> {
    const lanes: QueueLane[] = ["BOOKBER", "WALKIN"];
    let normalizedLanes = 0;
