...
export class RedisConsistencyValidator {
   ...
   */
  async validateQueueConsistency(shopId: string, lane: QueueLane): Promise<{
    isConsistent: boolean;
    dbQueueLength: number;
    redisQueueLength: number;
    mismatchedBookings: string[];
  }> {
    if (!this.redis) {
   ...
   */
  private async getMismatchedBookings(shopId: string, lane: QueueLane): Promise<string[]> {
    if (!this.redis) {
      return [];
    }
    ...
    const mismatchedBookings: string[] = [];
   ...
   */
  async validateWaitTimeConsistency(bookingId: string): Promise<{
    isConsistent: boolean;
    dbWaitMinutes: number;
    redisWaitMinutes: number | null;
  }> {
    if (!this.redis) {
   ...
   */
  async validateChairCacheConsistency(chairId: string): Promise<{
    isConsistent: boolean;
    dbStatus: string;
   ...
   */
  async validateShopQueues(shopId: string): Promise<{
    BOOKBER: Awaited<ReturnType<typeof RedisConsistencyValidator.prototype.validateQueueConsistency>>;
    WALKIN: Awaited<ReturnType<typeof RedisConsistencyValidator.prototype.validateQueueConsistency>>;
  }> {
   ...
   */
  async getShopHealthStatus(shopId: string): Promise<{
    isHealthy: boolean;
    queueConsistency: {
      BOOKBER: Awaited<ReturnType<typeof RedisConsistencyValidator.prototype.validateQueueConsistency>>;
      WALKIN: Awaited<ReturnType<typeof RedisConsistencyValidator.prototype.validateQueueConsistency>>;
    ...
    const totalInconsistencies =
      (queueConsistency.BOOKBER.isConsistent ? 0 : 1) +
