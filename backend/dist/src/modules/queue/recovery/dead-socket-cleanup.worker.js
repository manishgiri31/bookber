...
export class DeadSocketCleanupWorker {
    ...
    intervalMs;
    lock;
    constructor(lock, config = {}, intervalMs = 5 * 60 * 1000 // Default: 5 minutes
    ) {
     ...
     */
    async detectStaleSockets(socketManager) {
        const staleSockets = [];
     ...
     */
    async disconnectStaleSockets(staleSockets, socketManager) {
        ...
        for (const staleSocket of staleSockets) {
            try {
                ...
                await this.transactionManager.executeRecovery(() => this.logSocketCleanup(staleSocket), "SOCKET_DISCONNECTED", staleSocket.shopId || "global", "Disconnect stale socket");
                disconnected++;
            }
            catch (error) {
     ...
     */
    async logSocketCleanup(staleSocket) {
     ...
     */
    async disconnectSocket(socketId, socketManager) {
        if (!socketManager) {
     ...
     */
    async disconnectUserSockets(userId, socketManager) {
     ...
     */
    async disconnectShopSockets(shopId, socketManager) {
