...
export class RedisRepairService {
    ...
    intervalMs;
    lock;
    constructor(lock, config = {}, intervalMs = 30 * 60 * 1000 // Default: 30 minutes
    ) {
     ...
     */
    async detectRedisMismatches(redis) {
        const mismatches = [];
     ...
     */
    async detectRedisMismatchesForShop(shopId, redis) {
        const mismatches = [];
     ...
     */
    async detectRedisMismatchForLane(shopId, lane, redis) {
        // Get DB queue length
     ...
     */
    async detectBookingLevelMismatches(shopId, lane, redis) {
     ...
     */
    async rebuildRedisQueues(mismatches, redis) {
        ...
        for (const shopId of affectedShops) {
            await this.transactionManager.executeRecovery(() => this.rebuildShopRedisQueue(shopId, redis), "REDIS_REBUILT", shopId, "Rebuild Redis queue");
     ...
     */
    async rebuildShopRedisQueue(shopId, redis) {
        const lanes = ["BOOKBER", "WALKIN"];
        for (const lane of lanes) {
     ...
     */
    async rebuildLaneRedisQueue(shopId, lane, redis) {
