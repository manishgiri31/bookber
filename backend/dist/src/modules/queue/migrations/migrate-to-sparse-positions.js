...
export class SparsePositionMigration {
     ...
     */
    async migrateShop(shopId, redis) {
     ...
     */
    async migrateLane(shopId, lane, redis) {
     ...
     */
    async updateRedisPosition(redis, shopId, lane, bookingId, newPosition) {
        try {
     ...
     */
    async getLaneCount(shopId, lane) {
     ...
     */
    async validateMigration(shopId) {
        const lanes = ["BOOKBER", "WALKIN"];
        const issues = [];
