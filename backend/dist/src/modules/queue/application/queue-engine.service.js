...
export class QueueEngineService {
    ...
    waitTime;
    redisStore;
    constructor(repository, chairAllocator, lock, realtime, waitTime, redis = null) {
    ...
    laneForWalkIn(walkIn) {
        return walkIn ? "WALKIN" : "BOOKBER";
    }
    async reserveQueue(user, input) {
        if (input.walkIn && user.role === "CLIENT" && user.id !== input.userId) {
            throw Errors.forbidden();
        }
        const reserveStarted = process.hrtime.bigint();
        try {
            return await withSpan("queue.reserve", { shopId: input.shopId, walkIn: input.walkIn }, () => this.lock.withLock(`shop:${input.shopId}:reserve`, 8000, () => prisma.$transaction(async (tx) => {
                ...
                if (input.barberId) {
                    ...
                    if (!barber)
                        throw Errors.notFound("Barber not found in shop");
                }
                ...
                const recalc = await this.waitTime.recalculateLane(input.shopId, lane, WAIT_RECALC_TRIGGERS.ENQUEUE);
                ...
                await this.waitTime.persistence.applyEstimatesToDatabase(tx, input.shopId, recalc.estimates, durationMap);
                ...
                if (queueEntry) {
                    ...
                    getMetrics().bookingCreationDuration.observe({ shop_id: input.shopId, lane: queueEntry.lane }, durationSec);
    ...
    async completeService(user, bookingId) {
        try {
            return await this.lock.withLock(`booking:${bookingId}:complete`, 8000, () => prisma.$transaction(async (tx) => {
                await this.repository.lockBooking(tx, bookingId);
                const booking = await this.repository.findBooking(tx, bookingId);
                if (!booking)
                    throw Errors.notFound("Booking not found");
                ...
                const serviceStart = booking.chair?.activeServiceStart ?? booking.queueEntry?.estimatedServiceStart;
                const actualMinutes = serviceStart != null
    ...
    async rebalanceShop(shopId) {
        return this.lock.withLock(`shop:${shopId}:rebalance`, 8000, async () => prisma.$transaction(async (tx) => {
    ...
    async tryAssignNext(tx, shopId, lane) {
        ...
        getMetrics().queueAssignmentDuration.observe({ shop_id: shopId, lane }, Number(process.hrtime.bigint() - assignStarted) / 1e9);
    ...
    async startService(user, bookingId) {
        return prisma.$transaction(async (tx) => {
            ...
            if (!booking.chairId)
                throw Errors.conflict("No chair assigned");
    ...
    async compactQueue(tx, shopId, lane) {
        ...
        if (needsRebalancing || needsNormalization) {
            ...
            for (const entry of entries) {
                await this.redisStore.updatePosition(shopId, lane, entry.bookingId, entry.position, entry.estimatedWaitMinutes);
            }
        }
    }
    async rebalanceLane(tx, shopId, lane, trigger) {
        const recalc = await this.waitTime.recalculateLane(shopId, lane, trigger);
        const durationMap = new Map(recalc.estimates.map((e) => [e.bookingId, e.effectiveDurationMinutes]));
        await this.waitTime.persistence.applyEstimatesToDatabase(tx, shopId, recalc.estimates, durationMap);
        const entries = await this.repository.listActiveQueueEntries(tx, shopId, lane);
    ...
    async buildSnapshot(db, shopId, version) {
        const mapEntry = async (lane) => {
