...
export class PrismaAdminRepository {
    async getAnalyticsOverview(fromDate, toDate) {
        ...
        const [totalUsers, totalBarbers, totalShops, totalBookings, activeBookings, completedBookings, cancelledBookings, totalRevenue, averageRating, totalReviews] = await Promise.all([
    ...
    `;
      ...
      LIMIT 10
    `;
        const topShops = await prisma.$queryRaw `
      SELECT
        s.id as "shopId",
        s.name as "shopName",
        COUNT(b.id) as count,
    ...
    `;
        ...
        const revenueByShop = await prisma.$queryRaw `
      SELECT
        s.id as "shopId",
        s.name as "shopName",
        COALESCE(SUM(p.amount), 0) as revenue,
    ...
    `;
        const pendingPayouts = await prisma.$queryRaw `
      SELECT COUNT(*) as count
      FROM "Payment"
      WHERE status = 'PAID' AND "completedAt" IS NULL
    `;
        const completedPayouts = await prisma.$queryRaw `
      SELECT COUNT(*) as count
      FROM "Payment"
    ...
    async getAverageServiceDuration(shopId) {
        const aggregateArgs = { _avg: { durationMinutes: true } };
        if (shopId) {
