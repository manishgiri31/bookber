...
const DEFAULT_MEMORY_OPTIMIZER_CONFIG = {
...
export class RedisMemoryOptimizer extends EventEmitter {
     ...
     */
    async optimizeQueueMemory(shopId) {
        if (!this.redis) {
            return {
                success: false,
                memorySaved: 0,
     ...
     */
    async findKeysWithoutTTL(pattern) {
        if (!this.redis) {
            return [];
        }
        const keys = [];
     ...
     */
    async optimizeHashMemory(pattern) {
        if (!this.redis) {
            return {
                success: false,
                memorySaved: 0,
     ...
     */
    async compressHashField(key, field) {
        if (!this.redis) {
            return null;
        }
        try {
 ...
 */
export function createRedisMemoryOptimizer(redis, config) {
