...
const DEFAULT_CACHE_REBUILD_CONFIG = {
...
export class RedisCacheRebuildService extends EventEmitter {
     ...
     */
    async rebuildQueueCache(shopId, lane) {
        if (!this.redis) {
            return {
                success: false,
                keysRebuilt: 0,
     ...
     */
    updateStats(success, keysRebuilt, duration) {
 ...
 */
export function createRedisCacheRebuildService(redis, config) {
