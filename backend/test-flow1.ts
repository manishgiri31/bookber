...
import { ShopService } from "./src/modules/shop/application/shop.service.js";
...
async function testFlow1() {
  ...
  try {
    ...
    // Step 2: Create Barber profile
    console.log("\nStep 2: Create Barber profile");
    const barber = await prisma.barber.create({
      data: {
        userId: barberUser.id,
        shopId: "temp-shop-id" // Will be updated after shop creation
      }
    });
    ...
    // Step 4: Update Barber with correct shopId
    console.log("\nStep 4: Update Barber with correct shopId");
    const updatedBarber = await prisma.barber.update({
      where: { id: barber.id },
      data: { shopId: shop.id }
    });
    console.log("✓ Barber updated with shopId:", updatedBarber.shopId);
